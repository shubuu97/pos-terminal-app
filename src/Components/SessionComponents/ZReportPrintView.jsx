...
class ZReportPrintView extends React.Component {
            ...
            <div style={{ fontSize: "12px", fontFamily: "arial, sans-serif", padding: "50px" }}>
                <div style={{ textAlign: "center" }}>
                    ...
                    <div style={{borderBottom: 'solid 1px #9e9e9e', paddingBottom: '30px'}}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Difference:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.diffrence', 0).toFixed(2)}</span></div>
                    </div>
                    ...
                    <div style={{borderBottom: 'solid 1px #9e9e9e', paddingBottom: '30px'}}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Discounts:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.totalDiscountAmount', 0).toFixed(2)}</span></div>

                        <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Cash Refund:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.cashRefundAmount', 0).toFixed(2)}</span></div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Card Refund:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.cardRefundAmount', 0).toFixed(2)}</span></div>
                        {_get(this.props, 'data.giftCardRefundAmount', 0)!=0?<div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Gift Card Refund:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.giftCardRefundAmount', 0).toFixed(2)}</span></div>:null}
    ...
    {/* `                   <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Void:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.void', 0).toFixed(2)}</span></div> */}
                    ...
                    <div style={{borderBottom: 'solid 1px #9e9e9e', paddingBottom: '30px'}}>
                        ...
                        {_get(this.props, 'data.cashSalesAmount', 0)!==0?<div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Cash Payment:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.cashSalesAmount', 0).toFixed(2)}</span></div>:null}
                        {_get(this.props, 'data.cardSalesAmount', 0)!==0?<div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Credit/Debit Card:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.cardSalesAmount', 0).toFixed(2)}</span></div>:null}
                        {_get(this.props, 'data.employeeDeductSalesAmount', 0)!==0?<div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Employee Deduct:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.employeeDeductSalesAmount', 0).toFixed(2)}</span></div>:null}
                        {_get(this.props, 'data.costCenterChargeSalesAmount', 0)!==0?<div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Cost Center Charge:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.costCenterChargeSalesAmount', 0).toFixed(2)}</span></div>:null}
                        {_get(this.props, 'data.loyaltySalesAmount', 0)!==0?<div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Loyalty:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.loyaltySalesAmount', 0).toFixed(2)}</span></div>:null}
                        {_get(this.props, 'data.decliningBalanceSalesAmount', 0)!==0?<div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Declining Balance:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.decliningBalanceSalesAmount', 0).toFixed(2)}</span></div>:null}
                        {_get(this.props, 'data.giftCardSalesAmount', 0)!==0?<div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: "4px" }}>Gift Card:<span style={{fontWeight: 'bold' }}> {_get(this.props, 'data.giftCardSalesAmount', 0).toFixed(2)}</span></div>:null}
                    </div>
            </div>
        </div>
        );
